...
def update_package_xml(version, source_dir):
    ...
    new_content = re.sub(
        r"<version>.*?</version>", f"<version>{version}</version>", content
    )
    package_xml.write_text(new_content)

    # Check if there were changes
    result = run_command(
        ["git", "diff", "--quiet", "package.xml"], cwd=source_dir, check=False
    )
...
def update_pyproject_toml(version, source_dir, python_executable, cmake_module_dir):
    ...
    # Check if there were changes
    result = run_command(
        ["git", "diff", "--quiet", "pyproject.toml"], cwd=source_dir, check=False
    )
    if result.returncode != 0:
...
def update_changelog(version, source_dir):
    ...
    # Check if there were changes
    result = run_command(
        ["git", "diff", "--quiet", "CHANGELOG.md"], cwd=source_dir, check=False
    )
    if result.returncode != 0:
...
def update_pixi_toml(version, source_dir, python_executable, cmake_module_dir):
    ...
    # Check if there were changes
    result = run_command(
        ["git", "diff", "--quiet", "pixi.toml"], cwd=source_dir, check=False
    )
    if result.returncode != 0:
...
def create_git_tag(version, source_dir):
    ...
    run_command(
        ["git", "tag", "-s", tag_name, "-m", f"Release of version {version}."],
        cwd=source_dir,
    )
...
def run_dist_and_distclean(project_name, binary_dir):
    ...
    run_command(
        ["cmake", "--build", binary_dir, "--target", f"{project_name}-distclean"]
    )
...
def main():
    ...
    parser.add_argument(
        "--python-executable", required=True, help="Python executable path"
    )
    parser.add_argument(
        "--cmake-module-dir", required=True, help="CMake module directory path"
    )
